...
export class ReaderComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get(this.libraryService.backend + '/api/read/' + this.issueId).subscribe(res => {
      this.images = res as string[];
  ...
  backToComic() {
    this.loading = true;
    this.http.post(this.libraryService.backend + '/api/readingStatus', this.readingStatus).subscribe(res => {
      this.location.back();
  ...
  getImageUrl(image: string): string {
    if (this.issueId && image) {
      return this.libraryService.backend + "/api/image/" + this.issueId + "/" + encodeURIComponent(image);
    }
    return "";
  }
}
